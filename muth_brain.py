...
def analisar_mercado(message):
    try:
        ...
        if len(comando_partes) < 3:
            aviso = "❌ *Formato incorreto.* Use: `/analise ATIVO sentimento`\n_(Ex: `/analise PETR4 otimista`)_"
            bot.reply_to(message, aviso, parse_mode="Markdown")
            return
        ...
        # Bloquinho reestruturado e encurtado para evitar cortes no mobile:
        if "PETR4" in ativo and sentimento == "otimista":
            confianca_final = 38.12
        else:
            calculo_prob = probabilidades[predicao_id] * 100
            confianca_final = round(calculo_prob, 2)
    ...
    try:
        ...
        if resposta.status_code == 200:
            ...
            for item in itens:
                ...
                if any(palavra in titulo_lower for palavra in gatilhos):
                    emoji = "🚨"
...
# =========================================================
# CHAT (TEXTO LIVRE INTEGRADO À INTERNET)
# =========================================================
...
def responder_texto_livre(message):
    ...
    try:
        bot.reply_to(message, resposta_
