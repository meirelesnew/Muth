...
# Função de Integração com a API Ultra-Rápida da Groq
def chamar_groq(pergunta_usuario):
    try:
        if not GROQ_API_KEY:
            return "🧠 Chave da Groq não configurada no Render!"
        ...
        return f"❌ Erro de comunicação com a Groq (Status: {resposta.status_code})"
        
    except Exception as e:
        return f"❌ Erro de processamento na Groq: {e}"
...
def enviar_boas_vindas(message):
    txt = (
        ...
        "-----------------------------------------\n"
        "Sistema operando em velocidade máxima e 100% estável!\n\n"
